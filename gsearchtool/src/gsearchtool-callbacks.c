                           ...
                           gpointer   data)
{
  ...
  if (value != NULL) {
    g_settings_set_string (gsearch->mate_search_tool_settings,
                           "look-in-folder",
                           value);
  }
  g_free (value);
              ...
              gpointer     data)
{
  ...
  if (g_list_length (list) > SILENT_WINDOW_OPEN_LIMIT) {
    ...
    response = display_dialog_file_open_limit (gsearch->window,
                                               g_list_length (list));
                ...
                gpointer data)
{
  ...
  if (g_list_length (list) > SILENT_WINDOW_OPEN_LIMIT) {
    ...
    response = display_dialog_folder_open_limit (gsearch->window,
                                                 g_list_length (list));
  ...
  for (idx = 0; idx < g_list_length (list); idx++) {
    ...
    if (idx == 0) {
      ...
      g_file_info = g_file_query_info (g_file,
              ...
              gpointer   data)
{
  ...
  gtk_tree_model_get (GTK_TREE_MODEL (gsearch->search_results_list_store),
                                      &iter, COLUMN_LOCALE_FILE, &locale_path,
                                      -1);
                  ...
                  gpointer   data)
{
  ...
  for (idx = 0; idx < total; idx++) {
    ...
    gtk_tree_model_get (GTK_TREE_MODEL (gsearch->search_results_list_store),
                        &iter, COLUMN_NAME, &utf8_basename,
                           ...
                           gchar         *file)
{
  ...
  if (file == NULL || g_file_test (file, G_FILE_TEST_IS_DIR) == TRUE) {
    ...
    file_info = g_file_query_info (g_file,
                                   G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE,
                                   G_FILE_QUERY_INFO_NONE, NULL, NULL);
    list =
      g_app_info_get_all_for_type (g_file_info_get_content_type (file_info));
    ...
    if (list_length <= 0) {
      ...
      str = g_strdup_printf (_("_Open with %s"),
      ...
      gtk_container_add (GTK_CONTAINER (gsearch->search_results_popup_menu),
                         new1);
      g_signal_connect ((gpointer) new1,
                        "activate",
                        G_CALLBACK (open_file_cb),
                        (gpointer) gsearch);
  ...
  gsearch->search_results_save_results_as_item =
    mate_image_menu_item_new_from_icon  ("document-save", _("_Save Results As..."));
  gtk_container_add (GTK_CONTAINER (gsearch->search_results_popup_menu),
                     gsearch->search_results_save_results_as_item);
                              ...
                              gpointer        data)
{
  ...
  if (event->button == 1 || event->button == 2) {
    ...
    if (gtk_tree_view_get_path_at_pos (GTK_TREE_VIEW (gsearch->search_results_tree_view),
                                       event->x, event->y,
                                       &path, NULL, NULL, NULL)) {
  ...
  if (event->button == 3) {
    ...
    gtk_tree_model_get (GTK_TREE_MODEL (gsearch->search_results_list_store),
                        &iter, COLUMN_NAME, &utf8_name_first,
